  ...
  // Import our newly refactored hook
  ...
  const MainPlayground = () => {
    ...
    const { fetchSuggestion } = useInlineAI(); // Bring in our custom hook
    ...
    // 3. Fetch Template Data with Auto-Retry
    useEffect(() => {
      let isMounted = true;
    ...
    const handleEditorChange = async (newContent) => {
      if (activeFile) {
        ...
        setOpenFiles((prev) => prev.map((f) => (f.fullPath === activeFile.fullPath ? updatedFile : f)));
        ...
        saveTimerRef.current = setTimeout(async () => {
          try {
            await apiClient.patch(`/api/template/${id}/save`, { fullStructure: updatedTemplate.content });
          } catch (error) {}
        }, 1500); 
      }
    };

    // --- MONACO CONFIGURATION & NATIVE AI PROVIDER ---
  const handleEditorWillMount = (monaco) => {
      ...
      providerRef.current = monaco.languages.registerInlineCompletionsProvider('*', {
        provideInlineCompletions: (model, position, context, token) => {
          if (!isAiEnabledRef.current) return { items: [] };

          // --- EXTRACT FIM (Fill-In-The-Middle) CONTEXT ---
          const currentLine = model.getLineContent(position.lineNumber);
          
          // Don't trigger on empty lines with nothing around them
          if (currentLine.trim().length === 0 && position.lineNumber === 1) return { items: [] };

          const beforeContext = position.lineNumber > 1 
          ...
          const afterContext = position.lineNumber < model.getLineCount()
            ? model.getValueInRange({
                startLineNumber: position.lineNumber + 1, startColumn: 1,
                endLineNumber: model.getLineCount(),
                endColumn: model.getLineMaxColumn(model.getLineCount())
              })
            : "";

          // Determine basic AST clues (can be expanded later)
          const isAfterComment = currentLine.trim().startsWith('//') || currentLine.trim().startsWith('/*');
          const fileLanguage = getEditorLanguage(activeFile?.name || "");
          ...
          const fimPayload = {
            ...
            cursorPosition: { column: position.column - 1 }, // 0-indexed for substring
            afterContext: afterContext,
            isInFunction: beforeContext.includes('function') || beforeContext.includes('=>'),
            isInClass: beforeContext.includes('class '),
          ...
          return new Promise((resolve) => {
            aiDebounceTimer = setTimeout(async () => {
              ...
              console.log("🤖 AI Triggered! Fetching FIM completion...");
              
              let suggestionText = await fetchSuggestion(fimPayload);

              if (suggestionText) {
                // Prevent overlapping text on the current line
                const currentLinePrefix = currentLine.substring(0, position.column - 1);
                if (suggestionText.startsWith(currentLinePrefix)) {
                  suggestionText = suggestionText.slice(currentLinePrefix.length);
                }
