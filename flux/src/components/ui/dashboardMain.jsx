...
import {  Plus, Code, Zap, Cloud, GitBranch, Lock, RefreshCw,MoreHorizontal, Star, Eye, ExternalLink, 
  Pencil, Copy, Download, UserPlus, Trash2,X } from 'lucide-react';
    ...
    <div className="w-full  flex items-center justify-center px-6">
      ...
      <div className="flex flex-col items-center max-w-lg w-full">
        ...
        <div className="relative w-96 h-60 mb-8 anim-fade-1">
          ...
          <svg
            viewBox="0 0 24 24"
            className="absolute left-12 top-0 w-4.5 h-4.5 text-neutral-500 anim-twinkle"
            fill="currentColor"
          >
          ...
          <svg
            viewBox="0 0 24 24"
            className="absolute right-6 top-14 w-6 h-6 text-neutral-500 anim-twinkle-delay"
            fill="currentColor"
          >
            <path d="M12 0l1.8 8.2L22 10l-8.2 1.8L12 20l-1.8-8.2L2 10l8.2-1.8L12 0z" />
          </svg>
          ...
          <svg
            viewBox="0 0 64 40"
            className="absolute right-0 top-3 w-[84px] h-[54px] text-neutral-500 anim-float-slow"
            fill="none"
            stroke="currentColor"
            strokeWidth="1.5"
          >
            <path
              d="M18 30c-7 0-12-5-12-11 0-5.6 4.2-10.2 9.7-10.9C17.8 3 23.2 0 29 0c7.7 0 14 5.6 14.9 12.8C49.6 13.9 54 18.7 54 24.5 54 31 48.8 36 42.5 36H18z"
              strokeLinejoin="round"
            />
          </svg>
          ...
          <svg
            viewBox="0 0 56 46"
            className="absolute left-0 bottom-0 w-24 h-[78px] text-emerald-500 anim-float-delay"
            fill="none"
            stroke="currentColor"
            strokeWidth="1.5"
          >
            <path
              d="M2 8c0-1.657 1.343-3 3-3h13l4 5h29c1.657 0 3 1.343 3 3v27c0 1.657-1.343 3-3 3H5c-1.657 0-3-1.343-3-3V8z"
              strokeLinejoin="round"
              strokeLinecap="round"
            />
          </svg>
 
          {/* Code window stack (center, main element) — slightly tilted */}
          <div className="absolute left-1/2 top-9 -translate-x-1/2">
            {/* ghost card behind, opposite tilt, for depth */}
            ...
            <div className="relative w-60 h-42 rounded-xl border border-neutral-700 bg-[#111318] overflow-hidden -rotate-3 anim-float anim-glow">
              ...
              <div className="p-4 flex flex-col gap-3">
                <div className="flex items-center justify-center py-1">
                  <svg
                    viewBox="0 0 24 24"
                    className="w-8 h-8 text-neutral-400 shrink-0"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
        ...
        <p className="mt-3 text-center text-base leading-relaxed text-neutral-500 max-w-[420px] anim-fade-3">
          Create your first project to start coding, collaborate with others
          and build something amazing with flux.
        </p>
      </div>
    </div>
  );
...
const Avatar = ({ 
  pfpUrl, 
  initials = "U", 
  size = "w-10 h-13", // Added a default size for safety
  bgColor = "bg-teal-500" 
}) => {
  const [imgError, setImgError] = useState(false);

  // State: Image loaded successfully
  if (pfpUrl && !imgError) {
    return (
      // Changed 'rounded-full' to 'rounded-[12px]'
  ...
  return (
    <div
      className={`
        ...
        text-white font-medium tracking-wide flex-shrink-0
      `}
    >
      <span>{initials}</span>
    </div>
...
const actionCards = [
  ...
  },
  {
    ...
    features: [
      { icon: <GitBranch size={16} />, text: 'Import public or private repos' },
      { icon: <Lock size={16} />, text: 'Secure OAuth authentication' },
  ...
  return (
    <div className="bg-[#0F0F10] rounded-3xl p-6 flex flex-col border border-white/[0.05] relative overflow-hidden group">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <p className="text-[#8B949E] text-[14px] leading-relaxed mb-6">
          {card.description}
        </p>
        ...
        <button 
          onClick={onClick}
          className={`w-full py-2.5 px-6 rounded-xl cursor-pointer flex items-center justify-center gap-2 font-medium text-[14px] transition-all duration-200 ${currentTheme.button}`}
        >
          {card.buttonIcon}
          {card.buttonText}
        </button>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#1a1a1a] border border-white/10 rounded-xl w-[450px] p-6 shadow-2xl relative">
        ...
        <p className="text-[#8B929D] text-[13px] mt-1 mb-6">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <input 
              ...
              className="w-full bg-[#1a1a1a] border border-white/20 rounded-lg px-3 py-2.5 text-white text-[14px] outline-none focus:border-red-400 focus:ring-1 focus:ring-red-500 transition-all"
            />
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea 
              ...
              rows={4}
              className="w-full bg-[#1a1a1a] border border-white/20 rounded-lg px-3 py-2.5 text-white text-[14px] outline-none focus:border-red-400 focus:ring-1 focus:ring-red-500 transition-all resize-none"
            />
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 mt-8">
          <button 
            onClick={onClose} 
            className="px-5 py-2 text-[14px] font-medium cursor-pointer text-white bg-transparent border border-white/10 rounded-lg hover:bg-white/5 transition-colors"
          >
            Cancel
          </button>
          <button 
            ...
            className="px-5 py-2 bg-red-500 text-white text-[14px] font-semibold rounded-lg hover:bg-red-600 cursor-pointer transition-colors disabled:opacity-50"
          >
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#1a1a1a] border border-white/10 rounded-xl w-[450px] p-6 shadow-2xl">
        ...
        <div className="flex justify-end gap-3">
          <button 
            onClick={onClose} 
            className="px-5 py-2 text-[14px] font-medium cursor-pointer text-white border border-white/10 rounded-lg hover:bg-white/5 transition-colors"
          >
            Cancel
          </button>
          <button 
            disabled={isLoading}
            onClick={onConfirm} 
            className="px-5 py-2 bg-red-600 hover:bg-red-700 cursor-pointer text-white text-[14px] font-semibold rounded-lg transition-colors disabled:opacity-50"
          >
...
const AppCard = ({ app, isInitiallyStarred, onUpdateProject, onDeleteProject}) => {
  ...
  useEffect(() => {
    setFavourite(isInitiallyStarred);
  }, [isInitiallyStarred]);
  ...
  const formatDisplayDate = (dateStr) => {
    ...
    // If it's already in the "Jul 25, 2026" format (has a comma)
    if (typeof dateStr === 'string' && dateStr.includes(', ')) {
      const parts = dateStr.split(', ');
      return { part1: parts[0], part2: parts[1] };
    }
    
    // If it's a raw ISO string (like from the GitHub import API)
    try {
      ...
      const formatted = new Intl.DateTimeFormat('en-US', { 
        ...
        day: 'numeric', 
        year: 'numeric' 
      }).format(d); // Converts to "Jul 25, 2026"
  ...
  const handleMarkAsMyFavorite = async () => {
    setIsLoading(true);
    try {
      const newStatus = !favourite;
      await apiClient.post("/api/workspace/toggle-star", {
        workspaceId: app.id,
        isMarked: newStatus,
      });
  ...
  return (
    <div className="bg-[#0F0F10] rounded-[16px] p-6 flex flex-col justify-between h-full">
      ...
      <div>
        ...
        {/* Description text */}
        <p className="text-[#8B929D] text-[13px] leading-[1.4] mb-6 min-h-[40px] line-clamp-2">
          {app.description}
        </p>
      </div>
      ...
      <div>
        <div className="flex gap-5 mb-6">
          ...
          <div className="flex flex-col flex-1 mt-2 min-w-0">
            <div className="mb-1">
              <Avatar 
                pfpUrl={app.owner?.imageUrl} 
                initials={app.owner?.name ? app.owner.name[0].toUpperCase() : "U"}
                size="w-10 h-10" 
                bgColor="bg-teal-600" 
              />
            </div>
        ...
        <div className="flex justify-between items-center mt-auto">
          ...
          <div className="relative" ref={menuRef}>
            <button 
              ...
              className="text-[#8B929D] hover:text-white transition-colors cursor-pointer p-1 rounded-lg"
            >
            ...
            {isMenuOpen && (
              <div className="absolute top-full right-0 mt-1 w-[190px] bg-[#1a1a1a] border border-white/5 rounded-xl shadow-2xl z-50 p-1.5 flex flex-col gap-0.5">
                ...
                <button onClick={(e) => { e.stopPropagation(); handleMarkAsMyFavorite(); setIsMenuOpen(false); }} className="flex items-center gap-3 px-2.5 py-1.5 text-[13px] text-red-400 hover:bg-white/10 rounded-lg transition-colors w-full text-left">
      ...
      <EditProjectModal 
        ...
        initialData={{ title: app.title, description: app.description }}
        isLoading={isLoading}
        onConfirm={async (updatedData) => {
          await handleUpdateProject(updatedData);
        }}
      />
...
export default function App({ appData, setAppData }) {
  const { user } = useUser();
  const router = useRouter();
const [starredIds, setStarredIds] = useState([]);
  const [isStarredLoaded, setIsStarredLoaded] = useState(false); // Add this
  ...
  useEffect(() => {
    const fetchStarred = async () => {
      try {
        ...
        setIsStarredLoaded(true); // Mark as loaded even if it fails
      }
    };
    fetchStarred();
  }, []);
...
const handleDeleteProjectFromList = (id) => {
  // .filter() keeps all projects EXCEPT the one that matches the deleted ID
  ...
  const [isGithubModalOpen, setIsGithubModalOpen] = useState(false); // <-- Added Github Modal State
  
  // State to hold the template selected in step 1
...
const handleCreateProject = async ({ projectName, description }) => {
  setIsLoading(true);
  try {
    // 1. Prepare data - only include template if it exists
    const payload = {
      title: projectName,
    ...
    if (selectedTemplate?.name) {
      payload.template = selectedTemplate.name;
    }

    // 2. Call your API
    const response = await apiClient.post('/api/workspace', payload);

    // 3. Extract the workspace from your ApiResponse
    ...
    // Delay to prevent race condition before redirect
    await new Promise(resolve => setTimeout(resolve, 1500));

    // 5. Redirect to the dynamic route
...
// <-- FIX APPLIED HERE for Github Import Cache & Data Extraction
const handleImportGithub = async (repoUrl) => {
  setIsLoading(true);
  try {
    const response = await apiClient.post('/api/template/import', { repoUrl });
    
    // Safely extract the data regardless of the exact wrapper
    const newWorkspace = response.data?.data || response.data?.workspace || response.data; 

    if (newWorkspace && newWorkspace.id) {
      setAppData((prev) => [newWorkspace, ...prev]);
    } else {
      console.warn("API succeeded, but returned an unexpected data structure:", response.data);
    }
    
    handleCloseAll();
    toast.success("Repository imported successfully!");

    // Refresh the router to purge stale cache
    router.refresh(); 
  ...
  const handleCloseAll = () => {
    ...
    setIsGithubModalOpen(false); // <-- Ensure it closes
  ...
  const handleActionCardClick = (type) => {
    if (type === 'create') {
      ...
      setIsGithubModalOpen(true); // <-- Open Github Modal instead of console.log
    }
  };
